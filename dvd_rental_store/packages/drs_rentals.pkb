prompt - package body drs_rentals

create or replace package body drs_rentals as
  ...
  begin
    ...
    from
      drs_customer          c
      left join drs_address a on c.address_id = a.address_id
      left join drs_city    i on a.city_id    = i.city_id
      left join drs_country o on i.country_id = o.country_id
    where
  ...
  begin
    ...
    from
      drs_film               f
      left join drs_language l1 on f.language_id          = l1.language_id
      left join drs_language l2 on f.original_language_id = l2.language_id
    where
      f.film_id = p_film_id;
  ...
  begin
    ...
    from
      (
        ...
        from
          drs_film                f
          left join drs_inventory i on i.film_id      = f.film_id
          left join drs_rental    r on r.inventory_id = i.inventory_id
        where
  ...
  begin
    ...
    from
      drs_actor           a
      join drs_film_actor fa on fa.actor_id = a.actor_id
    where
    ...
    from
      drs_category           c
      join drs_film_category fc on fc.category_id = c.category_id
    where
